...
| EXP-013 | patient_info를 이력 마트 우선순위 규칙에서 분리 + 원천 테이블(op/ooodmordr) JOIN 유도 노트 추가 | ❌ 폐기 | Execution Accuracy 35.7%→39.3%(+3.6pp, 턴1은 39.1%→39.1% 무변화), 타겟 문항 2건의 schema mapping f1이 변경 전후 완전 동일 — schema_linking이 애초에 필요 테이블을 후보에 못 넣은 게 원인이라 SQL 생성 단계 notes 수정으론 못 고침 | [exp-013-patient-info-mart-note-correction.md](./detail/exp-013-patient-info-mart-note-correction.md) |
| EXP-014 | Schema Retrieval Recall(schema_linking)과 Schema Review Retention(schema_review) 병목 분리 진단(28턴 confusion matrix, 코드 변경 없음) | 📝 정성적 관찰 | Retrieval Recall 89.3%, Review Retention 76.0% — review_miss 6건 중 5건이 동일 패턴(poc_prostate_op가 검색은 됐지만 schema_review가 탈락시키고 원본 raw 테이블을 대신 선택). 원인: schema_review 프롬프트가 domain_notes(마트 우선 규칙)를 전혀 안 받음 | [exp-014-schema-retrieval-vs-review-bottleneck.md](./detail/exp-014-schema-retrieval-vs-review-bottleneck.md) |
| EXP-015 | schema_review의 테이블 재선정 프롬프트(`_SELECT_PROMPT`)에 domain_notes(마트 우선 규칙) 주입 | ✅ 채택 | Execution Accuracy 39.3%→50.0%(+10.7pp), 스키마매핑 F1 52.5%→77.5%(+25.0pp), review_miss 6건→3건(Review Retention 76.0%→88.0%) — EXP-014에서 진단한 병목과 정확히 일치하는 개선, retrieval_miss는 미변경(3건 그대로)으로 인과관계 확인 | [exp-015-schema-review-domain-notes-injection.md](./detail/exp-015-schema-review-domain-notes-injection.md) |
| EXP-016 | `app/sql/validator.py`의 CTE 별칭 오탐 수정(EXP-014에서 eval 스크립트에만 반영됐던 수정을 런타임 validator에도 이식) | ✅ 채택 | 단위 테스트 수준(결정론적)에서 CTE 오탐 100% 해결, 회귀 없음 확인 — golden set aggregate(38턴)는 15/38→13/38로 하락했으나 이 버그의 영향 범위(2/38)보다 코드 미변경 상태의 순수 샘플링 노이즈(12/38↔15/38, 동일 코드 재실행)가 더 커서 aggregate로는 순효과 검출 불가 | [exp-016-sql-validator-cte-false-positive.md](./detail/exp-016-sql-validator-cte-false-positive.md) |
| EXP-017 | 스키마 임베딩(`schema_indexer.py`)에 도메인팩(`table_retrieval_hints`) 검색 힌트 주입 — DB 코멘트가 너무 일반적이라 벡터검색이 구분 못 하던 근접 명명 테이블(CT/MRI vs 골스캔/PET vs 병리텍스트, 처방 vs 처방-수술매핑) 보완 | ✅ 채택 | retrieval_miss 4건 전부 top-5 hit로 전환(단위 수준 검증), Schema Mapping F1 77.9%→83.5%(+5.6pp), Execution Accuracy 34.2%→36.8%(+2.6pp, 하락 없음) — 단 retrieval_miss 4건 중 end-to-end 정답 전환은 0건, SQL 생성이 코드값(ADT001/CT001 등)을 여전히 몰라 별도 병목 확인 | [exp-017-schema-embedding-retrieval-hints.md](./detail/exp-017-schema-embedding-retrieval-hints.md) |
| EXP-018 | SQL 생성 `domain_notes`(`notes:`)에 코드값 vocabulary(exam_cd/ordr_cd 화이트리스트 + mark_rslt_val 판정 규칙) 주입 — EXP-017에서 임베딩에만 배선했던 것을 SQL 생성 프롬프트에도 연결 | ✅ 채택 | Execution Accuracy 36.8%→39.5%(+2.7pp), Schema Mapping F1 83.5%→89.3%(+5.8pp) — 타깃 hard 케이스 4건 중 1건(conv19) end-to-end 해결. 1차 시도에서 컬럼명 미명시로 conv33에 새 실패 유발, 컬럼명(ordr_cd) 명시로 재수정. 나머지 3건은 컬럼 선택·value_anchor 등 별개 원인으로 잔존 | [exp-018-domain-notes-value-vocabulary.md](./detail/exp-018-domain-notes-value-vocabulary.md) |
| EXP-019 | 스키마 검색(`schema_{domain}`)에 Qdrant dense+sparse(BM25) 하이브리드 도입 — `exam_cd`/`ADT001` 같은 정확 식별자를 dense 임베딩이 놓치는 문제를 RRF fusion으로 보완 | ✅ 채택 | Execution Accuracy 44.7%→63.2%(+18.5pp), Schema Mapping recall 92.1%→98.2%(+6.1pp), F1 81.6%→84.7%(+3.1pp), precision은 유지(78.9%→78.3%) — `fastembed` 신규 의존성 추가 | [exp-019-schema-search-sparse-bm25-hybrid.md](./detail/exp-019-schema-search-sparse-bm25-hybrid.md) |
